...
trait InterpreterPair[F[_[_]],G[_[_]],M[_]] {
  def left: Interpreter[F,M]
  def right: Interpreter[G,M]
}
object InterpreterPair {
  def apply[F[_[_]],G[_[_]],M[_]]( left: Interpreter[F,M], right: Interpreter[G,M]) = InterpreterPair_( left, right )
  case class InterpreterPair_[F[_[_]],G[_[_]],M[_]]( left: Interpreter[F,M], right: Interpreter[G,M] ) extends InterpreterPair[F,G,M]
  case class FinalPair[F[_[_]],G[_[_]],M[_]]( leftI: F[M], right: Interpreter[G,M], monad:Monad[M]) extends InterpreterPair[F,G,M] {
    val left = Interpreter( leftI )
  }
}
...
case class Final[F[_[_]],M[_] : Monad]( fm: F[M] ) {
  def and[G[_[_]]]( interpreter: Interpreter[G,M] ) : Interpreter[InterpreterPair.FinalPair[F,G,?[_]],M] =
    Interpreter[InterpreterPair.FinalPair[F,G,?[_]],M]( InterpreterPair.FinalPair( fm, interpreter, implicitly[Monad[M]]) )
}
...
object Interpreter {
  ...
  type and[A[_[_]],B[_[_]]] = ({
    ...
    type fin[X[_]] = InterpreterPair.FinalPair[A,B,X]
  })
}
